...
public class Spielflaeche extends JPanel {
	...
	public void paint(Graphics g) {
		...
		Image gras = Toolkit.getDefaultToolkit().getImage(getClass().getResource("bg.jpg"));
		Image mauer= Toolkit.getDefaultToolkit().getImage(getClass().getResource("images.jpg"));
		Spielfeld play = new Spielfeld(20, 20); // intern wird das spielfeld als array implementiert
		...
		for (int x = 0; x < 20; x++) {
			for (int y = 0; y < 20; y++) {
				...
				if (play.getObj(x, y) == 1) {		// zeichnet die Objekte 1
					g.drawImage(gras, x*41,y*41,40,40, null); // hier wird das bild "bg.jpg" gezeichnet, also Grasflaeche
